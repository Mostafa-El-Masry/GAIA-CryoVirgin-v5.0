...
export default function WealthAlerts({ overview }: Props) {
  ...
  if (story.netChange < 0) {
    ...
    alerts.push("This month your net stash is up. Even small positive moves accumulate over time.");
  }
  ...
  return (
    <section className={`${surface} p-4 md:p-5`}>
      ...
      <p className="mt-1 text-xs gaia-muted">
        A calm mirror of where you stand right now. No blame, no pressure - just soft signals
        about your buffer and how much interest is helping.
      </p>
      <ul className="mt-3 space-y-2 text-xs text-[var(--gaia-text-default)]">
        {uniqueAlerts.map((msg, idx) => (
          <li key={idx} className="flex gap-2 rounded-xl bg-[var(--gaia-surface-soft)] px-3 py-2 text-left">
            <span className="mt-0.5 inline-flex h-4 w-4 flex-none items-center justify-center rounded-full bg-[var(--gaia-contrast-bg)]/20 text-[9px] font-semibold text-[var(--gaia-text-strong)]">
              {idx + 1}
            </span>
            <span>{msg}</span>
          </li>
