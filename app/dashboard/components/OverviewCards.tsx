...
function scan(prefix: string, keys: string[]) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      for (const k of healthKeys) {
        ...
        if (Array.isArray(v?.weights) && v.weights.length)
          weight = v.weights[v.weights.length - 1]?.kg ?? weight;
        if (Array.isArray(v?.glucoseLogs) && v.glucoseLogs.length)
          glucose = v.glucoseLogs[v.glucoseLogs.length - 1]?.mgdl ?? glucose;
      }
    ...
    <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      <a href="/apollo" className="gaia-panel rounded-xl border p-4 shadow-sm">
        ...
        <div className="mt-1 text-sm opacity-80 truncate">
          {data?.latestApollo?.key || "—"}
        </div>
      </a>

      <a
        href="/instagram"
        className="gaia-panel rounded-xl border p-4 shadow-sm"
      >
        <div className="text-sm gaia-muted">Instagram</div>
      ...
      <a href="/health" className="gaia-panel rounded-xl border p-4 shadow-sm">
        ...
        <div className="mt-1 text-sm opacity-80">
          {data?.weight ?? "—"} kg • {data?.glucose ?? "—"} mg/dL
        </div>
      </a>
      ...
      <a href="/wealth" className="gaia-panel rounded-xl border p-4 shadow-sm">
        ...
        <div className="mt-1 text-sm opacity-80">
          {(data?.wealthSnapshot ?? 0).toLocaleString()}
        </div>
      </a>
