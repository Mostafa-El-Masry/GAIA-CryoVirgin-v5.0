...
const UserProfilePage: React.FC<UserProfilePageProps> = ({ params }) => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      const mockUserData = {
        username: `user_${id}`,
        avatar: `https://i.pravatar.cc/150?img=${parseInt(id) % 70}`, // Random avatar based on ID
        ...
        <div className="grid grid-cols-3 gap-1 border-t border-gray-700 pt-8">
          {Array.from({ length: user.postCount > 9 ? 9 : user.postCount }).map(
            (_, index) => (
              <div
                key={index}
                className="w-full h-32 bg-gray-800 flex items-center justify-center text-gray-500 text-xs"
              >
                Post {index + 1}
              </div>
            ),
